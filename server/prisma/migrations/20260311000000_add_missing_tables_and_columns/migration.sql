...
CREATE TABLE IF NOT EXISTS "PushToken" ("id" SERIAL PRIMARY KEY, "userId" INTEGER NOT NULL, "token" TEXT NOT NULL, "platform" VARCHAR(20) NOT NULL DEFAULT 'unknown', "createdAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(), CONSTRAINT "PushToken_token_key" UNIQUE ("token"), CONSTRAINT "PushToken_userId_fkey" FOREIGN KEY ("userId") REFERENCES users(id) ON DELETE CASCADE ON UPDATE NO ACTION);
