...
import {
  ...
  FiBarChart2, FiAlertCircle, FiUsers, FiLayers, FiSearch
} from "react-icons/fi";
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/system/teacher-dashboard/stats/${user.id}`, {
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "clamp(1rem, 4vw, 2.5rem)" }}>
        ...
        <div className="td-fade" style={{
          ...
          <p style={{ color: "#e0e7ff", fontSize: 15, margin: 0, lineHeight: 1.6 }}>
            Manage classes, create content, and review student progress.
          </p>
        </div>
