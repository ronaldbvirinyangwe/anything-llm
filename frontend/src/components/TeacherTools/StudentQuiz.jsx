...
function parseQuestions(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (questionMatch) {
      ...
      const marksInQuestion = questionText.match(
        /\[(\d+)\s*marks?\]\s*\*?\s*$/i
      );
      if (marksInQuestion) {
        marks = parseInt(marksInQuestion[1]);
        questionText = questionText
          .replace(/\[?\(?\d+\s*marks?\)?\]?\s*\*?\s*$/i, "")
          .trim();
      }
    ...
    if (
      /^(mark\s*scheme|marking\s*(guide|criteria|rubric))\s*:/i.test(trimmed)
    ) {
      inMarkScheme = true;
      continue;
    }
    ...
    const standaloneMarks = trimmed.match(
      /^\*?\*?\[?\(?\s*(\d+)\s*marks?\s*\)?\]?\*?\*?\s*$/i
    );
    if (standaloneMarks) {
...
export default function StudentQuiz() {
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      try {
        const res = await axios.get(`${API_BASE}/system/quiz/${quizCode}`);
        if (res.data.success) {
          setQuiz(res.data.quiz);
          const parsed = parseQuestions(res.data.quiz.content);
          setQuestions(parsed);
          setStatuses(
            Object.fromEntries(parsed.map((_, i) => [i, S.UNVISITED]))
          );
          setPhase("start");
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const handleFullscreenChange = () => {
      ...
      if (!isFullscreen && phase === "exam") {
        ...
        if (newV > maxTabSwitches) {
          alert(
            "🚨 Fullscreen exit violation limit exceeded. Submitting quiz."
          );
          submitQuiz(newV, false);
        } else {
    ...
    const warnUnload = (e) => {
      e.returnValue = "Progress may be lost.";
  ...
  const submitQuiz = async (
    violations = tabViolationsRef.current,
    isTimeExpired = false
  ) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${API_BASE}/system/student/submit-quiz`,
        payload
      );
  ...
  if (phase === "loading") {
    return (
      <div
        style={{ ...css.root, justifyContent: "center", alignItems: "center" }}
      >
  ...
  if (phase === "error") {
    return (
      ...
      >
        ...
        <button style={css.btnPrimary} onClick={() => navigate("/")}>
          Go Back
        </button>
      </div>
    );
    ...
    return (
      ...
      >
        <div style={css.startCard}>
          <div style={css.startIcon}>
            <ShieldIcon />
          </div>
          <h1
            style={{ fontSize: "2rem", color: "#e2e8f0", margin: "0 0 10px" }}
          >
            Secure Assessment
          </h1>
          <h2
          ...
          <div style={css.startGrid}>
            <div style={css.infoBox}>
              <span style={css.infoLabel}>Difficulty</span>
              <span style={css.infoVal}>{quiz.difficulty}</span>
            </div>
            <div style={css.infoBox}>
              <span style={css.infoLabel}>Time Limit</span>
              <span style={css.infoVal}>
                {quiz.timeLimit > 0 ? `${quiz.timeLimit} min` : "None"}
              </span>
            </div>
            <div style={css.infoBox}>
              <span style={css.infoLabel}>Focus Limit</span>
              <span style={css.infoVal}>{quiz.tabLimit} violations</span>
            </div>
          </div>
          ...
          <div style={css.securityNotice}>
            ...
            >
              <li>
                Opens in strict <strong>fullscreen mode</strong>.
              </li>
          ...
          >
            <button style={css.btnSecondary} onClick={() => navigate("/")}>
              Cancel
            </button>
            <button style={css.btnPrimary} onClick={startExam}>
              Enter Secure Mode
            </button>
          </div>
        </div>
  ...
  const isBookmarked =
    q && (statuses[q.id] === S.MARKED || statuses[q.id] === S.ANS_MARKED);
  ...
  if (phase === "submitting") {
    return (
      ...
      >
        <div style={css.spinner} />
        <p style={{ color: "#818cf8", fontSize: 16, fontWeight: 600 }}>
          Submitting securely…
        </p>
      </div>
    );
        ...
        <div style={css.resultWrap}>
          <div style={css.resultCard}>
            ...
            {(feedback.timeExpired || feedback.autoSubmitted) && (
              ...
              >
                ...
                {feedback.timeExpired
                  ? "Time Limit Reached. Auto-submitted."
                  : `Security Violations (${feedback.tabViolations}/${quiz.tabLimit}). Auto-submitted.`}
              </div>
            )}
            ...
            <div style={css.resGrid}>
              <div style={css.resStatCard}>
                <span style={{ ...css.resStatVal, color: "#e2e8f0" }}>
                  {feedback.earnedPoints}/{feedback.totalPoints}
                </span>
                <span style={css.resStatLabel}>Score</span>
              </div>
              <div style={css.resStatCard}>
                <span style={{ ...css.resStatVal, color: "#4ade80" }}>
                  {feedback.feedback.filter((f) => f.isCorrect).length}
                </span>
                <span style={css.resStatLabel}>Correct</span>
              </div>
              <div style={css.resStatCard}>
                <span style={{ ...css.resStatVal, color: "#818cf8" }}>
                  {Object.keys(answers).length}
                </span>
                <span style={css.resStatLabel}>Attempted</span>
              </div>
              <div style={css.resStatCard}>
                <span style={{ ...css.resStatVal, color: "#64748b" }}>
                  {questions.length - Object.keys(answers).length}
                </span>
                <span style={css.resStatLabel}>Skipped</span>
              </div>
            </div>
              ...
              {feedback.feedback.map((fb, i) => (
                ...
                >
                  <div style={css.fbHeader}>
                    ...
                    <span style={css.fbPoints}>
                      {fb.pointsEarned}/{fb.pointsPossible} pts
                    </span>
                  </div>
                  ...
                  {fb.correctAnswer && fb.isCorrect === false && (
                    <div style={{ fontSize: "14px", color: "#f87171" }}>
                      ...
                      <strong style={{ color: "#e2e8f0" }}>
                        <FormattedText>{fb.correctAnswer}</FormattedText>
                      </strong>
                    </div>
                  )}
                </div>
    ...
    <div style={css.root}>
        ...
        <div style={css.topRight}>
          {quiz.timeLimit > 0 && (
            <div
              style={{
                ...css.timer,
                ...(danger ? css.timerDanger : warning ? css.timerWarn : {}),
              }}
            >
              <ClockIcon />
                ...
                <div key={s} style={css.legendRow}>
                  <div
                    style={{
                      ...css.legendDot,
                      background: ST[s].bg,
                      border: `1.5px solid ${ST[s].border}`,
                    }}
                  />
              ...
              <div style={css.navGrid}>
                {questions.map((question, i) => {
                  ...
                  return (
                    ...
                    >
                      {pad(i + 1)}
                      {(st === S.MARKED || st === S.ANS_MARKED) && (
                        <span style={css.markDot} />
                      )}
                    </button>
                  );
                })}
            ...
            {q && (
              <>
                <div style={css.qHeader}>
                  ...
                  {q.marks && (
                    <span style={css.markGreen}>{q.marks} Marks</span>
                  )}
          ...
          <div style={css.rightInner}>
            ...
            <div style={css.overviewGrid}>
              {[
                ...
                { l: "Marked", v: stats.marked, c: "#a78bfa" },
              ].map(({ l, v, c }) => (
        ...
        <div style={css.overlay} onClick={() => setPhase("exam")}>
          <div style={css.modal} onClick={(e) => e.stopPropagation()}>
            <div style={css.modalHdr}>
              ...
              <button style={css.modalX} onClick={() => setPhase("exam")}>
                ✕
              </button>
            </div>
            <div style={css.modalBody}>
              <div style={css.confirmGrid}>
                <div style={css.confirmRow}>
                  <span style={css.confirmLabel}>Answered</span>
                  <span
                    style={{ fontSize: 22, fontWeight: 700, color: "#4ade80" }}
                  >
                    {stats.answered}
                  </span>
                </div>
                <div style={css.confirmRow}>
                  <span style={css.confirmLabel}>Unanswered</span>
                  <span
                    style={{ fontSize: 22, fontWeight: 700, color: "#f87171" }}
                  >
                    {stats.notAns + stats.notVisited}
                  </span>
                </div>
              </div>
              <p style={css.confirmWarn}>
              ...
              >
                ...
                <button style={css.btnPrimary} onClick={() => submitQuiz()}>
                  Submit Now
                </button>
              </div>
            </div>
      ...
      {showInstr && (
        <div style={css.overlay} onClick={() => setShowInstr(false)}>
          <div style={css.modal} onClick={(e) => e.stopPropagation()}>
            <div style={css.modalHdr}>
              <span style={css.modalTitle}>Exam Rules</span>
              <button style={css.modalX} onClick={() => setShowInstr(false)}>
                ✕
              </button>
            </div>
            <div style={css.modalBody}>
              <p style={css.confirmWarn}>
                Do not exit fullscreen. Switching tabs will trigger a violation
                warning and may auto-submit your exam.
              </p>
            </div>
          </div>
        </div>
...
const css = {
  ...
  optBtnSel: {
    background: "#1e1b4b",
    border: "1px solid #4338ca",
    boxShadow: "0 0 0 2px rgba(67,56,202,.2)",
  },
  optLetter: {
    width: 32,
    height: 32,
