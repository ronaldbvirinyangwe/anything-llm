...
const scoreColor = (s) =>
  s >= 80 ? "#16a34a" : s >= 60 ? "var(--theme-button-primary)" : "#dc2626";
      ...
      <div style={{ position: "relative", width: size, height: size }}>
        <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
          <circle
            ...
            cy={size / 2}
            r={r}
            stroke="var(--theme-sidebar-item-default)"
            strokeWidth={9}
            fill="none"
          />
          <circle
  ...
  <div className="pr-stat-card">
    ...
    <div
      style={{
        fontSize: 26,
        fontWeight: 800,
        color: accent || "var(--theme-text-primary)",
        lineHeight: 1,
      }}
    >
      {value}
  ...
  >
    ...
    >
      <div>
        ...
        <p style={{ color: "#64748b", margin: "4px 0 0", fontSize: "15px" }}>
          {child?.grade} · Term Report
        </p>
      </div>
      <div style={{ textAlign: "right" }}>
        <p style={{ fontSize: "13px", color: "#94a3b8", margin: 0 }}>
          Generated by Chikoro AI
        </p>
    ...
    >
      {[
        ...
        >
          <div style={{ fontSize: "28px", fontWeight: 700, color: "#46c8ff" }}>
            {s.value}
          </div>
...
const TABS = (struggledCount) => [
  ...
  {
    key: "struggles",
    label: `Needs Attention${struggledCount ? ` (${struggledCount})` : ""}`,
  },
...
export default function ParentReport() {
  ...
  const fetchNotificationSettings = async () => {
    try {
      setNotifSettings({ weekly: true, alerts: true, alertThreshold: 70 });
      const res = await fetch(
        `${API_BASE}/system/parent/notification-settings/${childId}`,
        {
          headers: {
      ...
      if (data.success && data.settings) {
        setNotifSettings({
          weekly: data.settings.weeklyDigest ?? true,
          alerts: data.settings.alertsEnabled ?? true,
          alertThreshold: data.settings.alertThreshold ?? 70,
        });
      }
    } catch (err) {
  ...
  const saveNotifSettings = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/system/parent/notification-settings`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
          ...
          body: JSON.stringify({
            childId,
            weeklyDigest: notifSettings.weekly,
            alertsEnabled: notifSettings.alerts,
            alertThreshold: notifSettings.alertThreshold,
          }),
        }
      );
      if (!res.ok) throw new Error(`Request failed with status ${res.status}`);
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      pdf.save(
        `${child?.name}_Report_${new Date().toISOString().slice(0, 10)}.pdf`
      );
    } catch (err) {
    ...
    return (
      ...
      >
        <div style={{ textAlign: "center" }}>
          ...
          />
          <p style={{ color: "var(--theme-text-secondary)", fontWeight: 600 }}>
            Loading report…
          </p>
        </div>
      </div>
  ...
  quizzes.forEach((q) => {
    ...
    if (!subjectStats[subj])
      subjectStats[subj] = { total: 0, count: 0, scores: [] };
  ...
  const subjectAverages = Object.keys(subjectStats).map((subj) => ({
    ...
    trend:
      subjectStats[subj].scores.length >= 2
  ...
  const heatmapData = Array.from({ length: 30 }, (_, i) => {
    ...
    return {
      ...
      score:
        dayQ.length > 0
          ? dayQ.reduce((s, q) => s + parseFloat(q.score), 0) / dayQ.length
          : null,
    };
  });

  // Struggled areas (all subjects flattened)
  const allStruggled = Object.entries(struggledAreas || {}).flatMap(
    ([subject, qs]) => qs.map((q) => ({ ...q, subject }))
  );
  ...
  const chartGridColor =
    getComputedStyle(document.documentElement)
      .getPropertyValue("--theme-sidebar-border")
      .trim() || "#333";
  const chartAccentColor =
    getComputedStyle(document.documentElement)
      .getPropertyValue("--theme-button-primary")
      .trim() || "#46c8ff";
        ...
        <div className="pr-modal-overlay">
          <div className="pr-modal">
            ...
            >
              ...
              weekly progress digest and low-score alerts sent straight to your
              inbox.
            </p>
              ...
              {[
                {
                  ...
                  text: (
                    <>
                      <strong>Weekly digest</strong> — every Sunday morning
                    </>
                  ),
                },
                {
                  ...
                  text: (
                    <>
                      <strong>Low-score alert</strong> — if {child?.name} scores
                      below {notifSettings.alertThreshold}%
                    </>
                  ),
                },
              ].map((item, i) => (
        ...
        >
              ...
              {[
                {
                  ...
                  desc: `Receive a summary of ${child?.name}'s progress every Sunday morning.`,
                },
                {
                  key: "alerts",
                  label: "Low score alerts",
                  desc: `Get notified if ${child?.name} scores below ${notifSettings.alertThreshold}% on any quiz.`,
                },
              ].map((item) => (
                <div
                  key={item.key}
          ...
          >
            ...
            <StatCard
              ...
              accent={scoreColor(averageScore)}
            />
            <StatCard
              emoji="📝"
              label="Quizzes Taken"
            ...
            <StatCard
              ...
              accent="var(--theme-button-primary)"
            />
            <StatCard
              emoji="🃏"
              label="Flashcard Mastery"
              value={`${mastered}/${totalFlashcards}`}
              sub="cards mastered"
            />
          </div>
          ...
          <div
            className="pr-card"
            style={{ padding: 0, overflow: "hidden", animationDelay: ".1s" }}
          >
            {/* Tab bar */}
            <div
              style={{
                ...
                overflowX: "auto",
                background:
                  "var(--theme-bg-sidebar, var(--theme-bg-secondary))",
              }}
            >
              {tabs.map((tab) => (
                <button
            ...
            <div style={{ padding: isMobile ? "20px 16px" : "28px 30px" }}>
                ...
                >
                  ...
                  <div>
                    ...
                    <div
                      style={{
                        ...
                        justifyContent: "center",
                        background:
                          "var(--theme-bg-container, var(--theme-bg-primary))",
                        border: "1px solid var(--theme-sidebar-border)",
                        borderRadius: 16,
                        padding: "28px 24px",
                      }}
                    >
                      ...
                      />
                      <RingProgress
                        value={Math.min(totalXP, 9999)}
                        max={Math.max(totalXP, 1000)}
                        label="XP Earned"
                        sublabel="pts"
                        color="#16a34a"
                      />
                      <RingProgress
                        value={totalFlashcards > 0 ? mastered : 0}
                        max={Math.max(totalFlashcards, 1)}
                        label="Flashcards Mastered"
                        sublabel={`/${totalFlashcards}`}
                        color="#f59e0b"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    {quizzes.length > 1 ? (
                      ...
                      >
                        More quizzes needed to show a trend.
                      </p>
                    )}
                  </div>
                ...
                >
                        ...
                        return (
                          <div key={s.subject} className="pr-subject-card">
                            ...
                            {s.trend !== null && (
                              ...
                              >
                                {s.trend >= 0 ? "▲" : "▼"}{" "}
                                {Math.abs(s.trend).toFixed(1)}% vs last quiz
                              </div>
                            )}
                          </div>
                        );
                  ...
                  <div>
                    ...
                    <Bar
                      data={histogramData}
                      options={baseChartOptions({
                        y: { beginAtZero: true, max: undefined },
                      })}
                    />
                  </div>
              ...
              {activeTab === "struggles" && (
                <div>
                  ...
                  >
                    These are questions {child?.name} got wrong. Expand each one
                    to see the correct answer and an explanation you can go
                    through together.
                  </p>
                  {allStruggled.length === 0 ? (
                    <div style={{ textAlign: "center", padding: "40px 0" }}>
                      ...
                      >
                        No struggles found — great work, {child?.name}!
                      </div>
                    </div>
                  ) : (
                    Object.entries(struggledAreas || {}).map(
                      ([subject, qs]) => (
              ...
              {activeTab === "home" && (
                <div>
                  ...
                  >
                    Personalised suggestions based on {child?.name}'s recent
                    performance.
                  </p>
                  ...
                  {homeAdvice ? (
                    <div
                      style={{
                        background:
                          "linear-gradient(135deg, rgba(22,163,74,.12) 0%, rgba(16,185,129,.08) 100%)",
